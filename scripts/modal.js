import { updateFilm, selectFilm, selectGenre, selectUserParams, selectProfilPictures, updateAvatar } from "./requetes.js";

let seasonsTotal = [];
export function modalCreation(type, title, year, trailer, backgroundModal, linkYtbBtn, videoFrame, modalTitle, seasonsSelect, episodesSelect, validateBtn, link, imdb, synopsisModal, synopsis, duree, background, api_key, genres) {
...
function displayPopUp(style, text, time) {
    if (activePopUp == false) {
        ...
        style == "success" ? divPopUp.style.backgroundColor = "rgba(25,135,84,1)" : divPopUp.style.backgroundColor = "rgba(255,0,0,1)";
        const maxZIndex = Math.max(
            ...Array.from(document.querySelectorAll('*'))
                .map((el) => parseFloat(getComputedStyle(el).zIndex))
                .filter((z) => !isNaN(z))
...
export function createModalUserParams(userInfo) {
    ...
    saveUserInfoBtn.classList.add("btn", "btn-success", "w-100", "saveUserInfoBtn");
        ...
        await fetch("../Bedflix/fonctions/update_user.php", {
            ...
            .then((result) => {
                if (result == "Erreur 1") {
                    ...
                    displayPopUp("error", "Le pseudo doit contenir au moins 3 caractères et ne doit contenir que des lettres et des espaces.", "5000");
                } else if (result == "Erreur 6") {
                    pLastnameSmall.textContent = "Invalide";
                    displayPopUp("error", "Le nom doit contenir au moins 2 caractères et ne doit contenir que des lettres et des espaces.", "5000");
                } else if (result == "Erreur 7") {
                    pFirstnameSmall.textContent = "Invalide";
                    displayPopUp("error", "Le prénom doit contenir au moins 2 caractères et ne doit contenir que des lettres et des espaces.", "5000");
                } else if (result == "Erreur 8") {
                    pPasswordSmall.textContent = "Invalide";
                    displayPopUp("error", "Le mot de passe doit contenir au moins 8 caractères, une majuscule et un caractère spécial.", "5000");
                } else if (result == "Erreur 9") {
                    pPasswordConfirmSmall.textContent = "Invalide";
                    displayPopUp("error", "Les mots de passe ne correspondent pas.", "5000");
                } else {
                    displayPopUp("success", "Vos informations ont été modifiées avec succès.", "3000");
                }
            });
    }
    ...
    avatarUserBtn.setAttribute("src", "../Bedflix/imgs/avatars/" + userInfo.photo_profil);
    ...
    avatarUserBtn.addEventListener("mouseleave", function () {
        ...
        avatarUserBtn.style.transition = "transform 200ms ease-in-out, boxShadow 200ms ease-in-out, opacity 200ms ease-in-out";
        avatarUserBtn.style.boxShadow = "rgb(255, 255, 255) 0.0625rem 0 0.625rem";
    });
...
export function createAvatarModal(userPicture, data) {
    ...
    avatarModalContent.style.width = "60%"
    ...
    modalTitle.textContent = "Choisissez votre avatar";
    modalTitle.style.color = "white";
    modalTitle.style.textShadow = "1px 1px 1px red"
    modalTitle.style.marginBottom = "1rem";
    modalTitle.style.borderBottom = "1px solid white";
    ...
    for (const category in categories) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            avatar1.addEventListener("click", function () {
                myAvatar.setAttribute("src", `../Bedflix/imgs/avatars/${images[i].url}`);
                document.body.querySelector(".avatarUserBtn").setAttribute("src", `../Bedflix/imgs/avatars/${images[i].url}`);
                document.body.querySelector("#avatar").setAttribute("src", `../Bedflix/imgs/avatars/${images[i].url}`);
            ...
            avatar1.addEventListener("mouseleave", function () {
                ...
                avatar1.style.transition = "transform 200ms ease-in-out, boxShadow 200ms ease-in-out, opacity 200ms ease-in-out";
                avatar1.style.boxShadow = "rgb(255, 255, 255) 0.0625rem 0 0.625rem";
            });
        }
    }
