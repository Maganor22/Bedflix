...
let myNote;
function createStarsClick(note, modalContent) {
  let deleteStars = document.querySelectorAll(".stars");
  for (let i = 0; i < deleteStars.length; i++) {
    if (note instanceof Object) {
      deleteStars[i].remove();
  ...
  const starHovered = (hoveredIndex) => {
    for (let i = 0; i < stars.children.length; i++) {
      const star = stars.children[i];
      myNote = hoveredIndex + 1;
      if (i <= hoveredIndex) {
        star.style.borderColor = "white";
  ...
  for (let i = 0; i < 5; i++) {
    ...
    star.addEventListener("mouseover", () => starHovered(i));
...
export function createComments(id, type, title, comment, modalContent) {
  ...
  commentLink.addEventListener("click", function () {
    getComments(id, type, title);
    //createCommentsModal(id, type, title);
  });
}

export function getBanned(data, id, type, title) {
  checkBanComment(data, id, type, title);
}

export async function createCommentsModal(
  dataBan,
  dataComments,
  id,
  type,
  ...
  if (dataBan == 1) {
    ...
    pBan.innerHTML = "Vous avez été banni, vous ne pouvez plus commenter.";
    pBan.style.color = "red";
    pBan.style.marginBottom = "2rem";
    commentsModalContent.appendChild(pBan);
  } else {
    const addComment = document.createElement("button");
    addComment.classList.add("btn", "btn-warning", "add-comment", "mb-3");
    addComment.style.width = "fit-content";
    addComment.innerHTML = "Ajouter un commentaire";
    addComment.style.marginBottom = "2rem";
    addComment.addEventListener("click", function () {
      createAddCommentModal(id);
    });
    commentsModalContent.appendChild(addComment);
  }
  ...
  // Parsez le retour de la requête JSON en utilisant JSON.parse
  let allDatas = JSON.parse(dataComments);

  allDatas.sort((a, b) => new Date(b.date) - new Date(a.date));

  // Vérifiez que data est bien un tableau
  if (Array.isArray(allDatas)) {
    // Parcourez les commentaires avec forEach
    allDatas.forEach((comment) => {
      ...
      } else if (comment.picture.includes("https")) {
        //supprime tout ce qu'il y a devant le deuxieme https:// pour refaire le lien
        let picture = comment.picture;
        let firstOccurrence = picture.indexOf("https");
        let secondOccurrence = picture.indexOf("https", firstOccurrence + 1);
        let url = "";
        if (secondOccurrence !== -1) {
          url = picture.substring(secondOccurrence);
        }
        commentAvatar.src = url;
      } else {
        commentAvatar.src = comment.picture;
      }
      commentAvatar.alt = "Avatar de " + comment.user;
      ...
      commentAuthor.innerHTML = `${comment.user} / ${dateFormatee}`;
      ...
      createStars(comment.note, commentContainer);

      let commentContent = document.createElement("p");
      commentContent.innerHTML = comment.commentaire;
...
function createAddCommentModal(id) {
  let modalAddCommentElements = document.querySelectorAll(
    ".modalAddCommentElements"
  );
  for (let i = 0; i < modalAddCommentElements.length; i++) {
    modalAddCommentElements[i].remove();
  }

  let addCommentModal = document.createElement("div");
  addCommentModal.classList.add("modal", "modalAddCommentElements");
  ...
  //console.log(maxZIndex);
  addCommentModal.style.zIndex = maxZIndex + 1;
  document.body.appendChild(addCommentModal);

  let addCommentsModalContent = document.createElement("div");
  addCommentsModalContent.classList.add(
    "modal-content-comment-modal",
    "m-auto"
  );
  addCommentsModalContent.style.backgroundColor = "#212529";
  addCommentsModalContent.style.boxShadow = "rgb(255, 255, 255) 1px 0 0.625rem";
  addCommentsModalContent.style.padding = "2rem";
  addCommentsModalContent.style.width = "30%";

  let addCommentTitle = document.createElement("h4");
  addCommentTitle.classList.add("modal-title-comment");
  addCommentTitle.innerHTML = `Ajouter un commentaire`;
  addCommentTitle.style.color = "white";
  addCommentTitle.style.marginBottom = "1rem";
  addCommentsModalContent.appendChild(addCommentTitle);
  ...
  addCommentTitle.appendChild(closeBtn);
  ...
  addCommentForm.style.flexDirection = "column";
  addCommentForm.style.alignItems = "center";

  let note = "0";

  createStarsClick(note, addCommentsModalContent);

  let captchaObj = randomCaptcha();

  let addCommentFormContent = document.createElement("textarea");
  addCommentFormContent.classList.add("mt-3");
  addCommentFormContent.id = "add-comment-form-content";
  addCommentFormContent.name = "add-comment-form-content";
  addCommentFormContent.placeholder = "Votre commentaire";
  addCommentFormContent.style.width = "100%";
  addCommentFormContent.style.height = "10rem";
  addCommentFormContent.style.marginBottom = "1rem";
  addCommentFormContent.style.borderRadius = "0.8rem";
  addCommentFormContent.style.padding = "0.5rem";
  addCommentFormContent.style.border = "1px solid #888";
  addCommentFormContent.style.backgroundColor = "#1a1a1a";
  addCommentFormContent.style.color = "white";
  addCommentFormContent.style.fontSize = "1rem";
  addCommentFormContent.style.textAlign = "center";
  addCommentFormContent.style.fontFamily = "Roboto";
  addCommentFormContent.style.fontWeight = "bold";
  addCommentFormContent.style.outline = "none";
  addCommentFormContent.style.boxShadow = "none";
  addCommentFormContent.style.transition = "all 0.3s ease-in-out";
  addCommentFormContent.addEventListener("mouseover", function () {
    addCommentFormContent.style.border = "1px solid #fff";
  });
  addCommentFormContent.addEventListener("mouseout", function () {
    addCommentFormContent.style.border = "1px solid #888";
  });
  addCommentFormContent.addEventListener("blur", function () {
    addCommentFormContent.style.border = "1px solid #888";
  });

  let addCommentFormSubmit = document.createElement("input");
  addCommentFormSubmit.type = "submit";
  addCommentFormSubmit.id = "add-comment-form-submit";
  addCommentFormSubmit.name = "add-comment-form-submit";
  addCommentFormSubmit.value = "Envoyer";
  addCommentFormSubmit.classList.add("btn", "btn-warning");
  addCommentFormSubmit.style.width = "100%";
  addCommentFormSubmit.style.marginBottom = "1rem";
  addCommentFormSubmit.style.borderRadius = "0.8rem";
  addCommentFormSubmit.style.padding = "0.5rem";
  addCommentFormSubmit.style.border = "1px solid #888";
  //addCommentFormSubmit.style.backgroundColor = "#1a1a1a";
  //addCommentFormSubmit.style.color = "white";
  addCommentFormSubmit.style.fontSize = "1rem";
  addCommentFormSubmit.style.textAlign = "center";
  addCommentFormSubmit.style.fontFamily = "Roboto";
  addCommentFormSubmit.style.fontWeight = "bold";
  addCommentFormSubmit.style.outline = "none";
  addCommentFormSubmit.style.boxShadow = "none";
  addCommentFormSubmit.style.transition = "all 0.3s ease-in-out";
  addCommentFormSubmit.addEventListener("mouseover", function () {
    addCommentFormSubmit.style.border = "1px solid #fff";
  });
  addCommentFormSubmit.addEventListener("mouseout", function () {
    addCommentFormSubmit.style.border = "1px solid #888";
  });
  addCommentFormSubmit.addEventListener("blur", function () {
    addCommentFormSubmit.style.border = "1px solid #888";
  });
  addCommentFormSubmit.addEventListener("click", function (e) {
    e.preventDefault();
    if (captchaObj.result != captchaInput.value) {
      displayAlert("alert-danger", "Le captcha est incorrect.");
    } else {
      addUserComment(id, myNote, addCommentFormContent.value);
    }
  });

  let captcha = document.createElement("div");
  captcha.classList.add("w-100");

  let captchaText = document.createElement("p");
  captchaText.innerHTML = "Veuillez remplir le captcha ci-dessous :";
  captchaText.style.color = "white";
  captchaText.style.marginBottom = "1rem";
  captcha.appendChild(captchaText);

  let divContainerCaptcha = document.createElement("div");
  divContainerCaptcha.classList.add(
    "d-flex",
    "justify-content-between",
    "gap-5"
  );

  captcha.appendChild(divContainerCaptcha);

  let captchaRandom = document.createElement("p");
  captchaRandom.classList.add("align-self-center");
  captchaRandom.id = "captcha-random";
  captchaRandom.style.color = "white";
  captchaRandom.style.marginBottom = "1rem";
  captchaRandom.innerHTML = captchaObj.captcha;
  //captcha.appendChild(captchaRandom);
  divContainerCaptcha.appendChild(captchaRandom);

  let captchaInput = document.createElement("input");
  captchaInput.classList.add("w-50");
  captchaInput.type = "captcha";
  captchaInput.id = "captcha-input";
  captchaInput.name = "captcha-input";
  captchaInput.placeholder = "Captcha";
  captchaInput.style.width = "100%";
  captchaInput.style.marginBottom = "1rem";
  captchaInput.style.borderRadius = "0.8rem";
  captchaInput.style.padding = "0.5rem";
  captchaInput.style.border = "1px solid #888";
  captchaInput.style.backgroundColor = "#1a1a1a";
  captchaInput.style.color = "white";
  captchaInput.style.fontSize = "1rem";
  captchaInput.style.textAlign = "center";
  captchaInput.style.fontFamily = "Roboto";
  captchaInput.style.fontWeight = "bold";
  captchaInput.style.outline = "none";
  captchaInput.style.boxShadow = "none";
  captchaInput.style.transition = "all 0.3s ease-in-out";
  captchaInput.addEventListener("mouseover", function () {
    captchaInput.style.border = "1px solid #fff";
  });
  captchaInput.addEventListener("mouseout", function () {
    captchaInput.style.border = "1px solid #888";
  });
  captchaInput.addEventListener("blur", function () {
    captchaInput.style.border = "1px solid #888";
  });
  //captcha.appendChild(captchaInput);
  divContainerCaptcha.appendChild(captchaInput);

  addCommentForm.appendChild(addCommentFormContent);
  addCommentForm.appendChild(captcha);
  addCommentForm.appendChild(addCommentFormSubmit);

  addCommentsModalContent.appendChild(addCommentForm);

  addCommentModal.appendChild(addCommentsModalContent);
}
...
function randomCaptcha() {
  ...
  return { captcha: captcha, result: total };
}

export function responseComment(data) {
  let modalCom = document.querySelector(".modalAddCommentElements");
  modalCom.style.display = "none";
  if (data == "true") {
    displayAlert(
      "alert-success",
      "Votre commentaire a bien été envoyé. Il sera ajouté après validation par un administrateur."
    );
  } else {
    // Expression régulière pour extraire le message d'erreur
    var regex = /Exception: ([^<]+)/;
    var match = regex.exec(data);

    if (match && match.length > 1) {
      displayAlert("alert-danger", "Vous avez été banni des commentaires.");
    } else {
      displayAlert(
        "alert-danger",
...
function displayAlert(alertClass, alertMessage) {
  ...
  style.innerHTML =
    ".alert { position: fixed; top: 10%; left: 50%; transform: translateX(-50%); z-index: 99999999999999; width: fit-content; opacity: 1; transition: opacity 0.5s ease-in-out; } .alert.sticky { position: fixed !important; top: 10% !important; } @media (max-width: 767px) { .alert { width: 80%; text-align: center; } }";
...
async function createActorsModal(
  id,
  type,
  title,
  backgroundModal,
  ...
  if (data != "") {
    ...
    try {
      const response = await fetch(
        `https://api.betaseries.com/${type}/characters?key=${api_key}&id=${id}`
      );
      const actors = await response.json();
      for (let i = 0; i < actors.characters.length; i++) {
        let actorInfo = actors.characters[i];

        const actorResponsePlus = await fetch(
          `https://api.betaseries.com/persons/person?id=${actorInfo.person_id}&key=${api_key}`
        );
        ...
        insertActorBase(
          ...
          age,
          dateFormatee,
          actorInfoPlus.person.poster,
          actorInfoPlus.person.description
        );
      }
  ...
  try {
    ...
    for (let i = 0; i < data.similars.length; i++) {
      ...
      if (show.images.poster != null) {
        ...
        showPoster.src = show.images.poster;
        ...
        showPoster.addEventListener("mouseover", function () {
          ...
          showPoster.style.boxShadow =
            "rgb(255, 255, 255) 0.0625rem 0 1.5625rem";
          afficherPopUp(showPoster, show.title, show.creation);
        });
...
async function createSimilarMoviesModal(
  id,
  backgroundModal,
  linkYtbBtn,
  videoFrame,
