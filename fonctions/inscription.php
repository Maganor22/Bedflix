        ...
        if ($data["email"] != $_POST["email"]) {
            if ($data["pseudo"] != $_POST["nickname"]) {
                ...
                if ($_POST["password"] == $_POST["passwordConfirmation"]) {
                    ...
                    if ($mail->sendMail($email, "Inscription Bedflix", $body, true)) {
                        header("Location: ../connexion-view.php?success=email_verif");
                    }
                } else {
                    $_SESSION["error"] = "Les mots de passe ne correspondent pas";
                    header("Location: ../inscription-view.php?failed=1");
                }
            } else {
                $_SESSION["error"] = "Le pseudo existe déja.";
                header("Location: ../inscription-view.php?failed=2");
            }
        } else {
            $_SESSION["error"] = "L'email existe déja.";
            header("Location: ../inscription-view.php?failed=3");
        }
    }
} else {
    $_SESSION["error"] = "Veuillez remplir tous les champs.";
    header("Location: ../inscription-view.php?failed=4");
}
